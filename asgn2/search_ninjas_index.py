"""Build a Terrier index over the Cranfield collection.

The preprocessing choices from PA1 (case folding, non-alphanumeric stripping,
stopword removal with the supplied list, Porter stemming) are repeated here, but
they are now Terrier's term pipeline rather than our own code, and each one is
switchable so the experiment driver can measure what it is worth.
"""
...
# Which record fields make up the indexed text. ".W" (the abstract) repeats the
# title in most documents; author and bibliography are names and journal
# references, which is why they are not in the default.
FIELD_SETS = {
    "text": ["text"],
...
STEMMERS = {"porter": "porter", "weakporter": "weakporter", "none": "none"}
...
def load_stopwords(path=STOPWORD_FILE):
    """Read the assignment's stopword list, normalized the way the text is.

    The file is UTF-8 with CRLF endings and holds a few mojibake entries
    (``herse"``, ``himse"``, ...); running each line through the same
    non-alphanumeric split that the tokens go through is what keeps those from
...
def build_index(fields="title_text", stemmer="porter", stopwords="assignment", overwrite=False, verbose=False):
    """Index the collection under one preprocessing variant.

    Returns ``(index_ref, seconds)``. An existing index is reused unless
    ``overwrite`` is set, in which case the reported time is 0.0 -- only a real
    build is timed, so the numbers in the report always come from actual work.
    """
...
def main():
    ...
    ap.add_argument("--fields", default="title_text", choices=sorted(FIELD_SETS))
